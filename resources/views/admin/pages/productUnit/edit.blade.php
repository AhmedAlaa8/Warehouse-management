...
@section('title')
      <title>Edit Product Unit | Dashboard</title>
@endsection
      ...
      <div class="col-12">
            <div class="card">
                  <div class="card-header bg-warning">
                        <h3 class="card-title float-right">تعديل وحدة منتج</h3>
                  </div>

                  <form class="form-horizontal" method="POST" action="{{ route('admin.productUnit.update',$productUnit) }}">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                                          ...
                                          <select class="form-control select2 select2-hidden-accessible product_id"
                                                ...
                                                @foreach ($products as $product)
                                                      <option value="{{ $product->id }}"
                                                            {{ $productUnit->product_id == $product->id ? 'selected' : '' }}>
                                                            {{ $product->name }}</option>
                                                @endforeach
                                          </select>
                              ...
                              <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                          <input type="text" class="form-control @error('count') {{ 'is-invalid' }} @enderror" name="count" id="count" value="{{$productUnit->count}}">
                              ...
                              <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                          <select class="form-control select2 select2-hidden-accessible small_unit_id"
                                                ...
                                                @foreach ($units as $unit)
                                                      <option value="{{ $unit->id }}"
                                                            {{ $productUnit->small_unit_id == $unit->id ? 'selected' : '' }}>
                                                            {{ $unit->name }}</option>
                                                @endforeach
                                          </select>
      ...
      <div class="row mb-2">
            ...
            <div class="col-sm-6">
                  <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">تعديل وحدة منتج</li>
                  </ol>
            </div>
      </div>
@endsection
